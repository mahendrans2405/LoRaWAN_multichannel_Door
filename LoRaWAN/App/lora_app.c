...
static void OnRxData(LmHandlerAppData_t *appData, LmHandlerRxParams_t *params)
{
  /* USER CODE BEGIN OnRxData_1 */
	Door_OnRxData(appData, params); //lora_app_door_config
...
static void SendTxData(void)
{
  /* USER CODE BEGIN SendTxData_1 */
	// 1. Check if LoRa is Busy
	    if (LmHandlerIsBusy()) {
	        return;
	    }

	    // 2. Check if we have actually JOINED the network.
	    if (LmHandlerJoinStatus() != LORAMAC_HANDLER_SET) {
	        return;
	    }

	    Door_SendTxData(&AppData);
...
static void OnTxData(LmHandlerTxParams_t *params)
{
  ...
  if ((params != NULL))
  {
    ...
    if (params->IsMcpsConfirm != 0)
    {
      ...
      APP_LOG(TS_ON, VLEVEL_M, "###### U/L FRAME:%04d | PORT:%d | DR:%d | PWR:%d", params->UplinkCounter,
              params->AppData.Port, params->Datarate, params->TxPower);

      APP_LOG(TS_ON, VLEVEL_M, " | MSG TYPE:");
      APP_LOG(TS_ON, VLEVEL_M, "\r\n###### ========== MCPS-Confirm =============\r\n");
    }
  }

         Door_OnTxData(params);  // lora_app_door_config


      APP_LOG(TS_OFF, VLEVEL_H,"###### U/L FRAME:%04d | PORT:%d | DR:%d | PWR:%d\r\n",params->UplinkCounter, params->AppData.Port,  params->Datarate, params->TxPower);
	...
	if (joinParams != NULL)
	  {
	    if (joinParams->Status == LORAMAC_HANDLER_SUCCESS)
	    {
	      ...
	      UTIL_TIMER_Stop(&TxTimer);     /* ← ADD for door */
	      UTIL_TIMER_Start(&TxTimer);    /* ← ADD for door*/

	      // Trigger the queue send logic immediately after joining
	      UTIL_SEQ_SetTask((1 << CFG_SEQ_Task_LoRaSendOnTxTimerOrButtonEvent), CFG_SEQ_Prio_0);
	    }
	    else
	    {
	      ...
	      /* Retry Join immediately */
	      APP_LOG(TS_OFF, VLEVEL_M, "Retrying Join...\r\n");
