...
void GetUniqueId(uint8_t *id)
{
  /* USER CODE BEGIN GetUniqueId_1 */
	...
	uint32_t devAddr = LL_FLASH_GetUDN();
	  if (devAddr == 0xFFFFFFFF)
	  {
	    devAddr = ((HAL_GetUIDw0()) ^ (HAL_GetUIDw1()) ^ (HAL_GetUIDw2()));
	  }

	  /* 1. Fill the FIRST 4 bytes with the DevAddr */
	  id[0] = (uint8_t)(devAddr >> 24);
	  id[1] = (uint8_t)(devAddr >> 16);
	  id[2] = (uint8_t)(devAddr >> 8);
	  id[3] = (uint8_t)(devAddr & 0xFF);

	  /* 2. Fill the LAST 4 bytes with the SAME DevAddr */
	  id[4] = (uint8_t)(devAddr >> 24);
	  id[5] = (uint8_t)(devAddr >> 16);
	  id[6] = (uint8_t)(devAddr >> 8);
	  id[7] = (uint8_t)(devAddr & 0xFF);

	  /* Log the result */
	  APP_LOG(TS_ON, VLEVEL_M, "Full DevEUI (Mirror of DevAddr): %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X\r\n",
	          id[0], id[1], id[2], id[3], id[4], id[5], id[6], id[7]);

	  return; /* Skip the default ST logic */
